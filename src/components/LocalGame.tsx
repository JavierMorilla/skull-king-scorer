...
export default function LocalGame({ onLeave, onOpenPrivacy }: LocalGameProps) {
  ...
  const handleResultsComplete = (newResults: Result[], updatedPlayers: Player[]) => {
    ...
    track('local_round_completed', { round: currentRound, players: players.length });
  };
  ...
  const handleNextRound = () => {
    if (currentRound < 10) {
      ...
      // Finalizar: Resetear puntuaciones y volver a empezar
      const resetPlayers = players.map(p => ({ ...p, score: 0 }));
      setPlayers(resetPlayers);
      setCurrentRound(1);
      setBids([]);
      setResults([]);
      track('local_game_restarted');
      saveGameState(resetPlayers, 1, 'BETTING', [], [], settings);
    }
  };
