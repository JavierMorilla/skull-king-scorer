...
interface BettingProps {
  ...
  onSubmitBid?: (bid: number, extraBet: number) => Promise<void>;
  onToggleHighlight?: (playerId: string, status: boolean) => Promise<void>;
  onNextPhase?: () => Promise<void>;
}

const PlayerBidRow = memo(({ player, bid }: { player: Player, bid?: Bid }) => (
  <motion.div
...
const WaitingPlayerRow = memo(({ player, hasBid }: { player: Player, hasBid: boolean }) => (
  <div key={player.id} className="flex items-center justify-between bg-[#1b2b3b] p-3 rounded-lg w-full max-w-xs mx-auto">
    <span className="font-sans text-[#d3e4fa]">{player.name}</span>
    {hasBid ? (
...
const BidButton = memo(({ num, selected, onClick }: { num: number, selected: boolean, onClick: (n: number) => void }) => (
  <motion.button
    onClick={() => onClick(num)}
    ...
    className={`aspect-square rounded-xl flex flex-col items-center justify-center border-2 transition-colors duration-200 group ${selected
        ? 'bg-[#362600] border-[#fabd04] shadow-[0_0_20px_rgba(250,189,4,0.3)]'
        : 'bg-[#0c1d2c] border-transparent hover:bg-[#1b2b3b]'
      }`}
  >
    <span className={`font-mono text-2xl transition-colors ${selected ? 'text-[#fabd04] font-bold text-glow' : 'text-[#d3e4fa] group-hover:text-[#fabd04]'}`}>
      {num}
    </span>
  </motion.button>
));

export default function Betting({ room, players, bids, currentPlayerId, onSubmitBid, onToggleHighlight, onNextPhase }: BettingProps) {
  const { t } = useLanguage();
  ...
  const hasBid = bids.some(b => b.playerId === activeUserId);

  const isHost = room.hostId === activeUserId || (!currentPlayerId && room.hostId === auth.currentUser?.uid);
  ...
  const handleConfirmBid = useCallback(async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        if (onSubmitBid) {
          await onSubmitBid(selectedBid, extraBet);
        } else {
  ...
  }, [isHost, allBidsIn, onNextPhase, room.id, room.currentRound]);

  const handleToggleHighlight = useCallback(async (playerId: string, currentStatus: boolean) => {
    try {
      if (onToggleHighlight) {
        await onToggleHighlight(playerId, currentStatus);
      } else {
  ...
  }, [onToggleHighlight, room.id, room.currentRound]);

  const handleBidClick = useCallback((num: number) => {
          ...
          <div className="mt-2 space-y-3 w-full">
            {players.map(p => (
              <PlayerBidRow
                key={p.id}
                player={p}
                bid={bids.find(b => b.playerId === p.id)}
              />
            ))}
          </div>
          {isHost ? (
        ...
        <div className="mt-2 space-y-2 w-full">
          {players.map(p => (
            <WaitingPlayerRow
              key={p.id}
              player={p}
              hasBid={bids.some(b => b.playerId === p.id)}
            />
          ))}
        </div>
      </motion.div>
    ...
    >
          ...
          <div className="grid grid-cols-3 gap-2">
            {[0, 10, 20].map(val => (
              <button
                ...
                className={`py-2 rounded-lg font-mono text-sm transition-colors ${extraBet === val
      ...
      <div className="mt-12">
        <button
          ...
          className="w-full bg-gradient-to-r from-[#fabd04] to-[#b68900] text-[#261a00] py-5 rounded-xl flex items-center justify-center gap-3 active:scale-[0.96] transition-transform shadow-xl disabled:opacity-50"
        >
