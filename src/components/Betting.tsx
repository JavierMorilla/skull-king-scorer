      ...
      return (
        ...
        >
          ...
          {isHost ? (
            ...
            <p className="mt-10 text-apricot/60 font-mono text-sm uppercase tracking-widest animate-pulse">{t('bet.waitCaptain')}</p>
          )}
        </motion.div>
      );
    }
  ...
  return (
    ...
    >
      ...
      <section className="mb-10 text-center relative">
        ...
        <p className="font-display italic text-apricot text-lg mb-1">{t('bet.phase')}</p>
        <h2 className="font-display font-bold text-4xl text-ice tracking-tight">{t('bet.round', { num: room.currentRound })}</h2>
      ...
      <div className="space-y-4">
        <label className="font-mono text-xs uppercase tracking-widest text-apricot ml-1">{t('bet.choose')}</label>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[0, 10, 20].map(val => (
              <button
                ...
                className={`py-2.5 min-h-11 rounded-lg font-mono text-sm transition-colors tabular-nums ${extraBet === val
                    ? 'bg-gold text-abyssal-deep font-bold'
                    : 'bg-dark-void text-ice hover:bg-cabin-slate'
                  }`}
              >
