...
const PlayerRow = memo(({ player, isHostUser, t, onKick }: { player: Player, isHostUser: boolean, t: any, onKick: (id: string) => void }) => (
  <div className="bg-cabin-slate p-4 rounded-xl border border-slate-mist/10 flex items-center justify-between group transition-all hover:bg-cabin-slate/80 shadow-md w-full">
    <div className="flex items-center gap-3">
      {isHostUser && (
        <div className="cursor-grab active:cursor-grabbing text-gold/40 hover:text-gold transition-colors p-2 w-11 h-11 flex items-center justify-center rounded-xl hover:bg-gold/5">
          <span className="material-symbols-outlined text-2xl">drag_indicator</span>
        </div>
      )}
      <div>
        <div className="flex items-center gap-2">
          <p className="font-sans font-bold text-lg text-ice">{player.name}</p>
    ...
    <div className="flex items-center gap-3">
      {player.isReady ? (
        <div className="flex items-center gap-2 bg-gold/10 px-3 py-1.5 rounded-full border border-gold/20 text-gold">
          <span className="material-symbols-outlined text-base" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
          <span className="font-mono text-xs font-bold">{t('lobby.ready')}</span>
        </div>
      ) : (
        <div className="flex items-center gap-2 bg-dark-void px-3 py-1.5 rounded-full border border-slate-mist/10 text-slate-mist">
          <span className="material-symbols-outlined text-base">hourglass_empty</span>
          <span className="font-mono text-xs font-bold">{t('lobby.waiting')}</span>
        </div>
      )}
      {isHostUser && player.id !== auth.currentUser?.uid && (
        <button
          onClick={() => onKick(player.id)}
          className="text-coral/85 hover:text-coral transition-colors p-2 w-11 h-11 flex items-center justify-center rounded-xl hover:bg-coral/10 active:scale-[0.95]"
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        ...
        {/* Empty Slot */}
        <div className="border border-dashed border-slate-mist/20 p-4 rounded-xl flex items-center justify-center group hover:border-gold/30 transition-all active:scale-[0.98] cursor-default">
          ...
          {[
            ...
            return (
              <div
                ...
                className={`relative p-3 rounded-xl border transition-all duration-200 flex flex-col gap-2 overflow-hidden group ${
                  isEnabled
                    ? `bg-cabin-slate ${option.border} shadow-md`
                    : 'bg-dark-void border-slate-mist/5'
                } ${isHost ? 'cursor-pointer hover:border-slate-mist/20' : 'cursor-default'}`}
              >
        ...
        <div className="max-w-2xl mx-auto flex flex-col gap-4">
          {!isHost ? (
            <button 
              ...
              className={`w-full py-4 rounded-xl font-sans font-bold text-lg shadow-xl active:scale-[0.97] transition-all flex items-center justify-center gap-3 ${currentPlayer?.isReady ? 'bg-cabin-slate text-ice border border-slate-mist/10' : 'bg-gradient-to-r from-gold to-gold-hover text-abyssal-deep'}`}
            >
            ...
            <button 
              onClick={handleStart}
              disabled={!allReady}
              className="w-full bg-gradient-to-r from-gold to-gold-hover text-abyssal-deep py-4 rounded-xl font-sans font-bold text-lg shadow-xl shadow-gold/10 active:scale-[0.97] transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
