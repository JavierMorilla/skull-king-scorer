...
const Switch = memo(({ checked, onChange, colorClass = "bg-[#fabd04]", disabled = false }: { checked: boolean, onChange: (c: boolean) => void, colorClass?: string, disabled?: boolean }) => (
  <button
    type="button"
    role="switch"
...
const PlayerRow = memo(({ player, isHostUser, t, onKick }: { player: Player, isHostUser: boolean, t: any, onKick: (id: string) => void }) => (
  <div className="bg-[#1b2b3b] p-5 rounded-xl flex items-center justify-between group transition-all hover:bg-[#2b3b4b]/50 shadow-sm w-full">
    <div className="flex items-center gap-4">
      {isHostUser && (
        <div className="cursor-grab active:cursor-grabbing text-[#fabd04]/40 hover:text-[#fabd04] transition-colors p-1">
