...
const Switch = memo(({ checked, onChange, colorClass = "bg-[#fabd04]", ariaLabel }: { checked: boolean, onChange: (c: boolean) => void, colorClass?: string, ariaLabel?: string }) => (
  <button
    type="button"
    role="switch"
...
const NumericalBonus = memo(({ 
  ...
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-3">
      <div className={`w-10 h-10 rounded-full ${iconBg} shadow-inner border border-[#fabd04]/20 flex items-center justify-center`}>
        <span className="text-2xl drop-shadow-md">{icon}</span>
      </div>
      <div className="text-left">
        <span className="font-sans font-bold text-[#d3e4fa] block">{label}</span>
        <span className="font-mono text-[10px] text-[#c4c6cc] uppercase">{subLabel}</span>
      </div>
    </div>
    <div className="flex items-center gap-2">
      <button 
        ...
        disabled={value <= 0}
        className="w-8 h-8 rounded-full bg-[#263647] flex items-center justify-center text-[#fabd04] disabled:opacity-30"
      >
            ...
            {room.settings?.characterBonusesEnabled && (
              <div className="mb-4 bg-[#0c1d2c] p-4 rounded-xl border border-[#fabd04]/20 relative z-10">
                ...
                <div className="space-y-4">
                  <label className="flex items-center justify-between cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#1b2b3b] flex items-center justify-center border border-[#ffb3ae]/20 shadow-inner">
